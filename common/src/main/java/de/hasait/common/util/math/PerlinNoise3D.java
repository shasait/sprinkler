...
public final class PerlinNoise3D {
	 ...
	 */
	public PerlinNoise3D(long pSeed, int pNoiseSize, int pCoordFact, int pOctaveCount, int pPersistencePart, int pRange, int pMin) {
		super();
	...
	public int perlinNoise(long pX, long pY, long pZ) {
		long total = 0;

		for (PerlinOctave octave : _octaves) {
			total = total + octave.octaveNoise(pX, pY, pZ);
		}
	...
	private static class PerlinOctave {
		...
		private int interpolatedNoise(long pX, long pY, long pZ) {
			...
			final long coordZ = pZ / _coordFact;
			final int coordZPart = (int) (Math.abs(pZ) % _coordFact);
			final int xfix = pX < 0 ? -1 : 1;
			final int yfix = pY < 0 ? -1 : 1;
			final int zfix = pZ < 0 ? -1 : 1;
			final int noise000 = noise(coordX, coordY, coordZ);
			final int noise100 = noise(coordX + xfix, coordY, coordZ);
			final int noise010 = noise(coordX, coordY + yfix, coordZ);
			final int noise110 = noise(coordX + xfix, coordY + yfix, coordZ);
			final int noise001 = noise(coordX, coordY, coordZ + zfix);
			final int noise101 = noise(coordX + xfix, coordY, coordZ + zfix);
			final int noise011 = noise(coordX, coordY + yfix, coordZ + zfix);
			final int noise111 = noise(coordX + xfix, coordY + yfix, coordZ + zfix);
			final int noiseX10 = InterpolationUtil.cosine(noise000, noise100, coordXPart, _coordFact);
			final int noiseX20 = InterpolationUtil.cosine(noise010, noise110, coordXPart, _coordFact);
			final int noiseX11 = InterpolationUtil.cosine(noise001, noise101, coordXPart, _coordFact);
			final int noiseX21 = InterpolationUtil.cosine(noise011, noise111, coordXPart, _coordFact);
			final int noiseY0 = InterpolationUtil.cosine(noiseX10, noiseX20, coordYPart, _coordFact);
			final int noiseY1 = InterpolationUtil.cosine(noiseX11, noiseX21, coordYPart, _coordFact);
			final int result = InterpolationUtil.cosine(noiseY0, noiseY1, coordZPart, _coordFact);
			return result;
		}

		private int noise(long pX, long pY, long pZ) {
			final int i = (int) ((pX + pY * 31 + pZ * 511) % _noiseSize);
			return _noise[i + _noiseSize];
		}

		private int octaveNoise(long pX, long pY, long pZ) {
			return (int) ((long) interpolatedNoise(pX * _frequency, pY * _frequency, pZ * _frequency) * _persistencePart
					/ _persistenceFact);
		}
